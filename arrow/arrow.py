...
class Arrow:
        ...
        try:
            if granularity == "auto":
                ...
                if diff < self._SECS_PER_MINUTE:
                    ...
                    return locale_cls.describe("minute", sign, only_distance=only_distance)
                    ...
                    return locale_cls.describe("hour", sign, only_distance=only_distance)
                elif diff < self._SECS_PER_DAY:
                    hours = sign * max(delta_second // self._SECS_PER_HOUR, 2)
                    return locale_cls.describe("hours", hours, only_distance=only_distance)
                    ...
                    return locale_cls.describe("days", days, only_distance=only_distance)

                elif diff < self._SECS_PER_WEEK * 2:
                    return locale_cls.describe("week", sign, only_distance=only_distance)
                elif diff < self._SECS_PER_MONTH:
                    weeks = sign * max(delta_second // self._SECS_PER_WEEK, 2)
                    return locale_cls.describe("weeks", weeks, only_distance=only_distance)

                elif diff < self._SECS_PER_MONTH * 2:
                    return locale_cls.describe("month", sign, only_distance=only_distance)
                    ...
                    return locale_cls.describe("year", sign, only_distance=only_distance)
                else:
                    years = sign * max(delta_second // self._SECS_PER_YEAR, 2)
                    return locale_cls.describe("years", years, only_distance=only_distance)
                ...
                return locale_cls.describe(granularity, delta, only_distance=only_distance)
                ...
                return locale_cls.describe_multi(timeframes, only_distance=only_distance)
    ...
    def dehumanize(self, input_string: str, locale: str = "en_us") -> "Arrow":
        ...
        for unit, unit_object in locale_obj.timeframes.items():
            ...
            for time_delta, time_string in strings_to_search.items():
                ...
                time_unit_to_change += (
                    "s" if time_unit_to_change[-1] != "s" else ""
                )
    ...
    def isocalendar(self) -> Tuple[int, int, int]:
        ...
        return tuple(self._datetime.isocalendar())
