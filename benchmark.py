...
DATETIME_OBJ = datetime.strptime(RFC3339_DATE_TIME[:-6], DATE_TIME_FORMAT).replace(
    tzinfo=pytz.FixedOffset(get_local_utc_offset() * -1)
)
TIME = time()
    ...
    for k in benchmarks:
        ...
        for func in k():
            if test_only:
                ...
                times = timeit.repeat(
                    "func()", setup="from __main__ import func", number=100000
                )
                t = min(times)
                mins.append(t)
